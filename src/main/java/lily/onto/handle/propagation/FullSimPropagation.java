...
public class FullSimPropagation {
	...
	private void simPropagation() {
		...
		candiTPSet = getSimTripleCandidate();
		
		sGUpdate = true;
		globeCvg=false;
		
		int cn = 0;
		while (sGUpdate && (cn < maxUpdateTime) && !globeCvg) {
			/**构造对偶图**/
			...
			/**更新候选相似三元组对**/
			updateTripleCandidate(goodTPSet,candiTPSet, edgeMap);
			/**全图相似度是否已经收敛**/
			globeCvg=isGlobalConverge(edgeMap);
			cn++;
		}
	}
	 ...
	 * 选择候选相似三元组
	 **************************************************************************/
	private Set getSimTripleCandidate() {
		long start = System.currentTimeMillis();// 开始计时
	 ...
	 **************************************************************************/
	@SuppressWarnings({"unchecked"})
	private Set updateTripleCandidate(Set goodTPSet, Set tPairSet, HashMap edgeMap) {
		long start = System.currentTimeMillis();// 开始计时
		int ct=0;
	 ...
	 **************************************************************************/
	private boolean isGlobalConverge(HashMap edgeMap) {
		boolean flag = false;
		double delta = 0;
	...
	private void dataStruProc()
	{
		...
		ArrayList s_tmpStm=new ArrayList();
		ArrayList t_tmpStm=new ArrayList();
		for (Iterator itx = s_stmList.iterator(); itx.hasNext();){
		...
		//randomInitialSim(0.00);//打乱初始相似度的测试
