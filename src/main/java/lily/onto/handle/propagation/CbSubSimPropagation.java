	...
	private boolean isGlobalConvergence() {
			...
			for (int j = 0; j < t_cnptNum; j++) {
				...
				for (Iterator it = edgeMap.entrySet().iterator(); it.hasNext();) {
					...
					if ((s_baseURI + s_cnptName[i]).equals(pairS.resA
							.toString())
							&& (t_baseURI + t_cnptName[j]).equals(pairS.resB
									.toString())) {
					...
					if ((s_baseURI + s_cnptName[i]).equals(pairP.resA
							.toString())
							&& (t_baseURI + t_cnptName[j]).equals(pairP.resB
									.toString())) {
						PairSim ps = new PairSim();
						ps.sim = pairP.sim0;
					...
					if ((s_baseURI + s_cnptName[i]).equals(pairO.resA
							.toString())
							&& (t_baseURI + t_cnptName[j]).equals(pairO.resB
									.toString())) {
						PairSim ps = new PairSim();
						ps.sim = pairO.sim0;
			...
			for (int j = 0; j < t_propNum; j++) {
				...
				for (Iterator it = edgeMap.entrySet().iterator(); it.hasNext();) {
					...
					if ((s_baseURI + s_propName[i]).equals(pairS.resA
							.toString())
							&& (t_baseURI + t_propName[j]).equals(pairS.resB
									.toString())) {
						PairSim ps = new PairSim();
						ps.sim = pairS.sim0;
						ps.support = pairS.simr;
					...
					if ((s_baseURI + s_propName[i]).equals(pairP.resA
							.toString())
							&& (t_baseURI + t_propName[j]).equals(pairP.resB
									.toString())) {
						PairSim ps = new PairSim();
						ps.sim = pairP.sim0;
						ps.support = pairP.simr;
					...
					if ((s_baseURI + s_propName[i]).equals(pairO.resA
							.toString())
							&& (t_baseURI + t_propName[j]).equals(pairO.resB
									.toString())) {
						PairSim ps = new PairSim();
						ps.sim = pairO.sim0;
						ps.support = pairO.simr;
		...
		for (int i = 0; i < s_cnptNum; i++) {
			for (int j = 0; j < t_cnptNum; j++) {
				...
				// 估计
				// cnptSimK1[i][j] = csim *
				// Math.pow((csup/maxsup),0.5)*Math.pow((cnum/maxnum),0.5);
		...
		for (Iterator it = upSet.iterator(); it.hasNext();) {
			...
			for (Iterator itx = s_Stm.iterator(); itx.hasNext();) {
				...
				for (Iterator ity = t_Stm.iterator(); ity.hasNext();) {
					...
					// 都不是元语的情况
						...
						if (typeA == typeB) {
							...
							if (rawPairs.contains(subAName + subBName)) {
								/* 已经包含,采用计算的相似度 */
								simS = ((Double) rawPairSim.get(subAName
										+ subBName)).doubleValue();
							} else {
								/* 没有包含,采用 */
								simS = getElmSim(subA, subB, typeA);
							}
					...
					// 都不是元语的情况
						...
						if (typeA == typeB) {
							...
							if (rawPairs.contains(propAName + propBName)) {
								/* 已经包含,采用计算的相似度 */
								simP = ((Double) rawPairSim.get(propAName
										+ propBName)).doubleValue();
							} else {
								/* 没有包含,采用 */
								simP = getElmSim(propA, propB, typeA);
							}
					...
					// 都不是元语的情况
						...
						if (typeA == typeB) {
							...
							if (rawPairs.contains(objAName + objBName)) {
								/* 已经包含,采用计算的相似度 */
								simO = ((Double) rawPairSim.get(objAName
										+ objBName)).doubleValue();
							} else {
								/* 没有包含,采用 */
								simO = getElmSim(objA, objB, typeA);
							}
					...
					if (simElmNum >= 2
							&& !rawTriples.contains(stAName + stBName)) {
						weight += 5.0;
						TriplePair tpair = new TriplePair();
						tpair.tripleA = stA;
						tpair.tripleB = stB;
		...
		for (Iterator it = tPairSet.iterator(); it.hasNext();) {
			...
			for (Iterator itx = edgeMap.entrySet().iterator(); itx.hasNext();) {
				...
				if (!fS && sAName.equals(subAName) && sBName.equals(subBName)) {
					...
					if (pairS.cFlag && pairS.sim0 > 0.001) {
						sGUpdate = true;
					}
					fS = true;
				}
				if (!fO && oAName.equals(objAName) && oBName.equals(objBName)) {
					...
					if (pairO.cFlag && pairO.sim0 > 0.001) {
						sGUpdate = true;
					}
					fO = true;
				}
				if (!fP && pAName.equals(propAName) && pBName.equals(propBName)) {
					...
					if (pairP.cFlag && pairP.sim0 > 0.001) {
						sGUpdate = true;
					}
					fP = true;
				}
