...
public class LargeOntSimPropagation {
	/* 数据成员 */
...
//	public PropertySubGraph[] t_propSubG;// 属性子图
	...
	/** ****相似度矩阵***** */
	public SparseDoubleMatrix2D cnptSimRaw;// 原始概念相似度
	public SparseDoubleMatrix2D cnptSimK0;// k次迭代相似度
	public SparseDoubleMatrix2D cnptSimK1;// k+1次迭代相似度
	public SparseDoubleMatrix2D propSimRaw;// 原始属性相似度

	public SparseDoubleMatrix2D propSimK0;// k次迭代相似度
	public SparseDoubleMatrix2D propSimK1;// k+1次迭代相似度
	public SparseDoubleMatrix2D insSimRaw;// 原始实例相似度
	...
	private int maxProgTimes = 4;// 传播最大迭代次数
...
//		System.out.println("相似度传播算法时间：" + (double) costtime / 1000. + "秒");
	...
	private void unPackPara(ArrayList paraList) {
		...
		s_stmList = new ArrayList();//和小本体的不一致，设置为空
		...
		t_stmList = new ArrayList();//和小本体的不一致，设置为空

		t_baseURI = (String) (paraList.get(17));

		cnptSimRaw = (SparseDoubleMatrix2D) (paraList.get(18));
		propSimRaw = (SparseDoubleMatrix2D) (paraList.get(19));
		insSimRaw = (SparseDoubleMatrix2D) (paraList.get(20));
		...
		s_cnptStm = (ArrayList)(paraList.get(27));
		s_propStm = (ArrayList)(paraList.get(28));
		t_cnptStm = (ArrayList)(paraList.get(29));
		t_propStm = (ArrayList)(paraList.get(30));
	...
	private void initPara() {
		...
		cnptSimRaw = new SparseDoubleMatrix2D(s_cnptNum,t_cnptNum);
		propSimRaw = new SparseDoubleMatrix2D(s_propNum,t_propNum);
		insSimRaw = new SparseDoubleMatrix2D(s_insNum,t_insNum);
		...
		cnptSimK0 = new SparseDoubleMatrix2D(s_cnptNum,t_cnptNum);
		cnptSimK1 = new SparseDoubleMatrix2D(s_cnptNum,t_cnptNum);
		propSimK0 = new SparseDoubleMatrix2D(s_propNum,t_propNum);
		propSimK1 = new SparseDoubleMatrix2D(s_propNum,t_propNum);
	}
...
//		System.out.println("consTriplePairGraph时间：" + (double) ( end - start) / 1000. + "秒");
	...
	private void propagation(HashMap edgeMap, HashMap progWeightMap) {
		 ...
		 */
		while (k < maxProgTimes && delta < 0.9) {
			double max = 1.0;// 最大相似度
	...
	private boolean isLocalConverge(HashMap edgeMap) {
		...
		for (int i = 0; i < s_cnptNum; i++) {
			for (int j = 0; j < t_cnptNum; j++) {
				...
				if (EVMap.containsKey(key)){
					cnptSimK1.setQuick(i,j,(Double)(EVMap.get(key)));
				}
			}
		}
		...
		for (int i = 0; i < s_propNum; i++) {
			for (int j = 0; j < t_propNum; j++) {
				...
				if (EVMap.containsKey(key)){
					propSimK1.setQuick(i,j,(Double)(EVMap.get(key)));
				}				
			}
		}
		...
		if (fromCnpt){
			for (int i = 0; i < s_cnptNum; i++) {
				for (int j = 0; j < t_cnptNum; j++) {
					vA.add(cnptSimK0.getQuick(i,j));
					vB.add(cnptSimK1.getQuick(i,j));
					cnptSimK0.setQuick(i,j,cnptSimK1.getQuick(i,j));//同时更新相似度
				}
			}
		}		
		...
		if (fromProp){
			for (int i = 0; i < s_propNum; i++) {
				for (int j = 0; j < t_propNum; j++) {
					vA.add(propSimK0.getQuick(i,j));
					vB.add(propSimK1.getQuick(i,j));
					propSimK0.setQuick(i,j,propSimK1.getQuick(i,j));//同时更新相似度
				}
			}
		}
...
//		}
		...
		// 做一个事先的过滤
		cnptSimK0 = new SimpleFilter().maxValueFilter(s_cnptNum,t_cnptNum,cnptSimK0,0.005);
		propSimK0 = new SimpleFilter().maxValueFilter(s_propNum,t_propNum,propSimK0,0.005);
		...
		for (int i = 0; i < s_cnptNum; i++) {
			for (int j = 0; j < t_cnptNum; j++) {
				vA.add(cnptSimK0.getQuick(i,j));
				vB.add(cnptSimRaw.getQuick(i,j));
				cnptSimRaw.setQuick(i,j,cnptSimK0.getQuick(i,j));// 同时更新相似度
			}
		}
		...
		for (int i = 0; i < s_propNum; i++) {
			for (int j = 0; j < t_propNum; j++) {
				vA.add(propSimK0.getQuick(i,j));
				vB.add(propSimRaw.getQuick(i,j));
				propSimRaw.setQuick(i,j,propSimK0.getQuick(i,j));// 同时更新相似度
			}
		}
...
//		System.out.println("全局相似矩阵的迭代收敛相似度:" + delta);
	...
	private void dataStruProc()
	{
		...
		/**1.克隆原始相似度**/
		cnptSimK0 = (SparseDoubleMatrix2D)cnptSimRaw.copy();
		propSimK0 = (SparseDoubleMatrix2D)propSimRaw.copy();
		...
		for (int i = 0; i < s_cnptNum; i++) {
			...
			for (int j = 0; j < t_cnptNum; j++) {
				...
				cnptSimMap.put(s_cnptName[i] + t_cnptName[j], cnptSimRaw.getQuick(i,j));
				if (cnptSimRaw.getQuick(i,j)>0.001){
		...
		for (int i = 0; i < s_propNum; i++) {
			...
			for (int j = 0; j < t_propNum; j++) {
				...
				propSimMap.put(s_propName[i] + t_propName[j], propSimRaw.getQuick(i,j));
				if (propSimRaw.getQuick(i,j)>0.001){
		...
		for (int i = 0; i < s_insNum; i++) {
			...
			for (int j = 0; j < t_insNum; j++) {
				...
				insSimMap.put(s_insName[i] + t_insName[j], insSimRaw.getQuick(i,j));
				if (insSimRaw.getQuick(i,j)>0.005){
